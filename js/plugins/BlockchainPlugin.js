(function() {
  ...
  async function getContractAddress() {
    if (window.ethereum) {
      ...
      return contractAddresses[network.chainId] || contractAddresses[10143];
    }
    return contractAddresses[10143]; // Default to Monad Testnet
  }
  ...
  function attachFunctions() {
    ...
    $gameSystem.setKittens = async function(kittens) {
      ...
      try {
        ...
        console.log("Before fetch", { url: 'https://monad-cats-game.vercel.app/api/setKittens.js', body: JSON.stringify({ kittens: maxNewKittens, userAddress, chainId: chainIdNum }) });
        ...
        let response;
        try {
          response = await fetch('https://monad-cats-game.vercel.app/api/setKittens.js', {
            method: 'POST',
    ...
    $gameSystem.connectWallet = async function() {
      ...
      try {
        ...
        if (!isValidChain(network.chainId)) {
          $gameMessage.add("Please switch to Monad Testnet (10143).");
          $gameVariables.setValue(12, 0);
          return;
        }
    ...
    $gameSystem.fundContract = async function(ethAmount) {
      ...
      try {
        ...
        const network = await provider.getNetwork();
        if (!isValidChain(network.chainId)) {
          $gameMessage.add("Please switch to Monad Testnet.");
          return;
        }
